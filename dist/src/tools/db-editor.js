...
async function askNumber(query, defaultVal) {
    const a = await ask(query);
    if (!a.trim() && defaultVal !== undefined)
        return defaultVal;
    const n = Number(a);
    return isNaN(n) ? undefined : n;
}
async function askBoolean(query, defaultVal) {
...
async function askEnum(query, values, defaultVal) {
    console.log(`  Options: ${values.join(", ")}`);
    const a = (await ask(query)).trim();
    if (!a && defaultVal !== undefined)
        return defaultVal;
...
async function askStringArray(query, validValues) {
    const a = (await ask(query)).trim();
    if (!a)
        return undefined;
...
async function editFields(entry, fields, prefix = "") {
    const result = { ...entry };
    for (const f of fields) {
        const key = f.key;
        const prompt = (def) => `  ${f.label} (${def}): `;
        if (f.type === "object") {
        ...
        if (f.type === "string") {
            ...
            if (n !== undefined)
                ...
                if (result.hasOwnProperty(key) && currentVal === undefined)
                    delete result[key];
            }
        }
            ...
            console.log(`  ${f.label} current: ${currentVal?.join(", ") ?? "(none)"}`);
            if (f.values)
                console.log(`  Valid values: ${f.values.join(", ")}`);
            const a = await ask(`  ${f.label} (comma-separated, Enter to keep): `);
            if (a.trim()) {
                const items = a.split(",").map((s) => s.trim()).filter(Boolean);
                if (f.values) {
                    const invalid = items.filter((i) => !f.values.includes(i));
                    if (invalid.length > 0) {
...
async function displayEntry(entry, indent = "") {
...
// ── CRUD Operations ────────────────────────────────────────────────
async function listEntries(dbName) {
...
async function viewEntry(dbName) {
    const data = loadDb(dbName);
...
async function createEntry(dbName) {
    const data = loadDb(dbName);
    const meta = DATABASES[dbName];
    const entry = {};
    ...
    if (data[edited.id]) {
        console.log(`Entry "${edited.id}" already exists.`);
        return;
    }
...
async function editEntry(dbName) {
    const data = loadDb(dbName);
    const meta = DATABASES[dbName];
...
async function deleteEntry(dbName) {
    const data = loadDb(dbName);
...
async function dbMenu(dbName) {
    while (true) {
        ...
        switch (choice.trim()) {
            case "1":
                await listEntries(dbName);
                break;
            case "2":
                await viewEntry(dbName);
                break;
            case "3":
                await createEntry(dbName);
                break;
            case "4":
                await editEntry(dbName);
                break;
            case "5":
                await deleteEntry(dbName);
                break;
            case "6": return;
