...
exports.itemEffects = exports.abilityEffects = exports.moveEffects = void 0;
...
exports.moveEffects = {
    ...
    'effect:sunny_day': (ctx) => {
        ctx.engine.setWeather('HARSH_SUNLIGHT');
    },
    'effect:dark_burst': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.CURSED, 4);
    },
    'effect:blinding_flash': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.BLINDED, 3);
    },
    'effect:piercing_claw': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.PIERCED, 3);
    },
    'effect:vital_light': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.VITALIZED, 4);
    },
    'effect:curse_web': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.STACKED, 0);
    },
    'effect:skill_crush': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applySecondaryStatus(t, types_1.SecondaryStatus.SKILL_DRAINED, 3, ctx.user);
    },
    'effect:charm_glow': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applySecondaryStatus(t, types_1.SecondaryStatus.CHARMED, 3, ctx.user);
    },
    'effect:spectral_chain': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applySecondaryStatus(t, types_1.SecondaryStatus.CHAINED, 3, ctx.user);
    },
    'effect:life_drain': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applySecondaryStatus(t, types_1.SecondaryStatus.LIFE_DRAINED, 4, ctx.user);
    },
    'effect:tether_bind': (ctx) => {
        if (ctx.targets.length >= 2) {
            ctx.engine.applySecondaryStatus(ctx.targets[0], types_1.SecondaryStatus.TETHERED, 3, ctx.targets[1]);
            ctx.engine.applySecondaryStatus(ctx.targets[1], types_1.SecondaryStatus.TETHERED, 3, ctx.targets[0]);
        }
    },
    ...
    'effect:life_conversion_sacrifice': (ctx) => {
        ...
        if (userMon.currentHP <= 0)
            engine.faintMon(ctx.user);
    },
    'effect:set_zombie_world': (ctx) => {
        ctx.engine.setField('ZOMBIE_WORLD');
    },
    'effect:set_megalith_portal': (ctx) => {
        ctx.engine.setField('MEGALITH_PORTAL');
    },
    ...
    'effect:will_o_wisp': (ctx) => {
        for (const t of ctx.targets)
            ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.BURN, 0);
    },
    ...
    'effect:play_rough': (ctx) => {
        for (const t of ctx.targets) {
            if (Math.random() < 0.1)
                ctx.engine.modifyStatStage(t, 'atk', -1);
        }
    },
    'effect:zen_headbutt': (ctx) => {
        for (const t of ctx.targets) {
            if (Math.random() < 0.2)
                ctx.engine.applySecondaryStatus(t, types_1.SecondaryStatus.FLINCHED, 1, ctx.user);
        }
    },
    ...
    'effect:shadow_ball': (ctx) => {
        for (const t of ctx.targets) {
            if (Math.random() < 0.2)
                ctx.engine.modifyStatStage(t, 'spDef', -1);
        }
    },
    ...
    'effect:hurricane': (ctx) => {
        for (const t of ctx.targets) {
            if (Math.random() < 0.3) {
                ...
                ctx.engine.applySecondaryStatus(t, types_1.SecondaryStatus.CONFUSED, turns, ctx.user);
            }
        }
    },
    ...
    'effect:blizzard': (ctx) => {
        for (const t of ctx.targets) {
            if (Math.random() < 0.1)
                ctx.engine.applyPrimaryStatus(t, types_1.PrimaryStatus.PARALYSIS, 0);
        }
    },
    ...
    'effect:tailwind': (ctx) => {
        ...
        for (let i = 0; i <= 2; i++) {
            ...
            if (ally)
                engine.modifyStatStage({ side: ctx.user.side, index: i }, 'spe', 2);
        }
    },
...
exports.abilityEffects = {
    ...
    'ability:zombie_lord': (ctx) => {
        const mon = ctx.engine.state[ctx.source.side].active[ctx.source.index];
        if (mon && mon.primaryStatus === types_1.PrimaryStatus.NONE) {
            ctx.engine.applyPrimaryStatus(ctx.source, types_1.PrimaryStatus.VITALIZED, 0);
    ...
    'ability:intimidate': (ctx) => {
        ...
        for (let i = 0; i <= 2; i++) {
            ...
            if (t && t.currentHP > 0)
                ctx.engine.modifyStatStage({ side: opp, index: i }, 'atk', -1);
        }
...
exports.itemEffects = {
    ...
    'item:flame_orb': (ctx) => {
        const mon = ctx.engine.state[ctx.holder.side].active[ctx.holder.index];
        if (mon && mon.primaryStatus === types_1.PrimaryStatus.NONE) {
            ctx.engine.applyPrimaryStatus(ctx.holder, types_1.PrimaryStatus.BURN, 0);
    ...
    'item:toxic_orb': (ctx) => {
        const mon = ctx.engine.state[ctx.holder.side].active[ctx.holder.index];
        if (mon && mon.primaryStatus === types_1.PrimaryStatus.NONE) {
            ctx.engine.applyPrimaryStatus(ctx.holder, types_1.PrimaryStatus.POISON, 0);
    ...
    'item:white_herb': (ctx) => {
        const mon = ctx.engine.state[ctx.holder.side].active[ctx.holder.index];
        if (mon) {
            const hasNeg = Object.values(mon.statStages).some((v) => v < 0);
            if (hasNeg) {
    ...
    'item:black_sludge': (ctx) => {
        ...
        const isFitting = reg && reg.types.some((t) => t === types_1.MonsterType.ZOMBIE || t === types_1.MonsterType.FIEND);
        if (isFitting) {
            const heal = Math.floor(mon.maxHP / 16);
            mon.currentHP = Math.min(mon.maxHP, mon.currentHP + heal);
            ...
            if (mon.currentHP <= 0)
                ctx.engine.faintMon(ctx.holder);
        }
    },
};
